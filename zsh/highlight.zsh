...
typeset zsh_highlight_aliases="$(builtin alias -Lm '[^+]*')"
...
# Load highlighters from the "highlighters" directory.
_zsh_highlight_load_highlighters() {
  setopt localoptions noksharrays bareglobqual

  # Check the path to the "highlighters" directory exists
  if [[ ! -d "$1" ]]; then
...
_zsh_highlight_load_highlighters "${${0:A}:h}/highlighters"
if [[ $? -ne 0 ]]; then
...
eval "$zsh_highlight_aliases"
builtin unset zsh_highlight_aliases
